...
export class FirebaseAnalyticsProvider implements AnalyticsProvider {
  ...
  identify(userId: string, _properties?: UserProperties): void {
    ...
    if (_properties) {
      ...
      firebaseAnalytics.setUserProperties(userProps).catch(() => {});
    }
  }

  reset(): void {
    if (!firebaseAnalytics) return;
    firebaseAnalytics.setUserId(null).catch(() => {});
    firebaseAnalytics.resetAnalyticsData().catch(() => {});
  }

  setEnabled(enabled: boolean): void {
    this.enabled = enabled;
    if (firebaseAnalytics) {
      firebaseAnalytics.setAnalyticsCollectionEnabled(enabled).catch(() => {});
    }
  }
}
