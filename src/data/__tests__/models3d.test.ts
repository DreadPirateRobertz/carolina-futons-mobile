...
describe('models3d', () => {
  describe('MODELS_3D catalog', () => {
    ...
    it.each(MODELS_3D)('$productId has reasonable file size (1 KB - 20 MB)', (asset: Model3DAsset) => {
  ...
  describe('getModel3DForProduct', () => {
    ...
    it('returns undefined for a product without AR model', () => {
      expect(getModel3DForProduct(productId('prod-grip-strips'))).toBeUndefined();
    });

    it('returns undefined for non-existent product', () => {
      expect(getModel3DForProduct(productId('prod-does-not-exist'))).toBeUndefined();
    });
  });

  describe('hasARModel', () => {
    it('returns true for futon products', () => {
      expect(hasARModel(productId('prod-asheville-full'))).toBe(true);
      expect(hasARModel(productId('prod-blue-ridge-queen'))).toBe(true);
      expect(hasARModel(productId('prod-pisgah-twin'))).toBe(true);
      expect(hasARModel(productId('prod-biltmore-loveseat'))).toBe(true);
    });

    it('returns true for murphy bed products', () => {
      expect(hasARModel(productId('prod-murphy-queen-vertical'))).toBe(true);
      expect(hasARModel(productId('prod-murphy-full-horizontal'))).toBe(true);
      expect(hasARModel(productId('prod-murphy-queen-bookcase'))).toBe(true);
      expect(hasARModel(productId('prod-murphy-twin-cabinet'))).toBe(true);
      expect(hasARModel(productId('prod-murphy-queen-desk'))).toBe(true);
      expect(hasARModel(productId('prod-murphy-full-storage'))).toBe(true);
    });

    it('returns true for frame product', () => {
      expect(hasARModel(productId('prod-hardwood-frame'))).toBe(true);
    });

    it('returns false for covers, pillows, accessories', () => {
      expect(hasARModel(productId('prod-mountain-cover-full'))).toBe(false);
      expect(hasARModel(productId('prod-arm-pillows'))).toBe(false);
      expect(hasARModel(productId('prod-grip-strips'))).toBe(false);
      expect(hasARModel(productId('prod-furniture-polish'))).toBe(false);
    });

    it('returns false for non-existent product', () => {
      expect(hasARModel(productId('fake-product'))).toBe(false);
    });
  });
