...
const mockGamificationAddToCart = jest.fn().mockResolvedValue({ success: true });
jest.mock('@/hooks/useGamificationEvents', () => ({
  useGamificationEvents: () => ({
    addToCart: mockGamificationAddToCart,
...
describe('gamification events on addItem', () => {
  beforeEach(() => {
    clearEventBuffer();
    mockGamificationAddToCart.mockClear();
  });

  it('calls gamification addToCart when a product is added', () => {
    const { getByTestId } = renderCart();
    fireEvent.press(getByTestId('add-asheville-linen'));
    expect(mockGamificationAddToCart).toHaveBeenCalled();
  });

  it('passes product_id to gamification addToCart', () => {
    const { getByTestId } = renderCart();
    fireEvent.press(getByTestId('add-asheville-linen'));
    expect(mockGamificationAddToCart).toHaveBeenCalledWith(asheville.id, expect.any(Number));
  });

  it('passes price (basePrice + fabric.price) to gamification addToCart', () => {
    const { getByTestId } = renderCart();
    fireEvent.press(getByTestId('add-asheville-linen'));
    // asheville basePrice=349, naturalLinen fabric.price=0 → total 349
    expect(mockGamificationAddToCart).toHaveBeenCalledWith(
      asheville.id,
      asheville.basePrice + naturalLinen.price,
    );
  });

  it('calls gamification addToCart for each distinct addItem call', () => {
    const { getByTestId } = renderCart();
    fireEvent.press(getByTestId('add-asheville-linen'));
    fireEvent.press(getByTestId('add-blueridge-blue'));
    expect(mockGamificationAddToCart).toHaveBeenCalledTimes(2);
  });
});
