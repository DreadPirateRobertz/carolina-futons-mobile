...
describe('useFabricTransition', () => {
  ...
  it('sets active fabric on select', async () => {
    const { result } = renderHook(() => useFabricTransition());
    await act(async () => result.current.selectFabric('mountain-blue'));
  ...
  it('selected swatch has scale > 1', async () => {
    const { result } = renderHook(() => useFabricTransition());
    await act(async () => result.current.selectFabric('mountain-blue'));
  ...
  it('unselected swatch has scale of 1', async () => {
    const { result } = renderHook(() => useFabricTransition());
    await act(async () => result.current.selectFabric('mountain-blue'));
  ...
  it('switching fabrics updates scales', async () => {
    const { result } = renderHook(() => useFabricTransition());
    await act(async () => result.current.selectFabric('mountain-blue'));
    expect(result.current.getSwatchScale('mountain-blue').value).toBeGreaterThanOrEqual(1);

    await act(async () => result.current.selectFabric('sunset-coral'));
  ...
  it('provides border opacity for selected vs unselected', async () => {
    const { result } = renderHook(() => useFabricTransition());
    await act(async () => result.current.selectFabric('charcoal'));
