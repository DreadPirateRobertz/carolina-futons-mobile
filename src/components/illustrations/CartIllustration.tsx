...
export function CartIllustration({ width = VBW, height = VBH, testID }: Props) {
  return (
    <Svg width={width} height={height} viewBox={`0 0 ${VBW} ${VBH}`} testID={testID}>
      ...
      {/* 5 mountain layers */}
      <Path
        d={buildSmallMountainPath(VBW, VBH, 0.55, 42)}
        fill={colors.mountainBlueDark}
        opacity={0.2}
      />
      <Path
        d={buildSmallMountainPath(VBW, VBH, 0.62, 17)}
        fill={colors.mountainBlue}
        opacity={0.3}
      />
      <Path d={buildSmallMountainPath(VBW, VBH, 0.7, 73)} fill={colors.espresso} opacity={0.35} />
      <Path
        d={buildSmallMountainPath(VBW, VBH, 0.78, 29)}
