...
describe('ARComparisonOverlay', () => {
  it('renders both model names', () => {
    const { getByText } = render(<ARComparisonOverlay modelA={modelA} modelB={modelB} />);
    expect(getByText('Queen')).toBeTruthy();
    expect(getByText('Full')).toBeTruthy();
  });

  it('renders both model test IDs', () => {
    const { getByTestId } = render(<ARComparisonOverlay modelA={modelA} modelB={modelB} />);
  ...
  it('shows shorter when B is taller', () => {
    ...
    const { getByText } = render(<ARComparisonOverlay modelA={modelA} modelB={tallerB} />);
    expect(getByText('-6" shorter')).toBeTruthy();
  });

  it('renders nothing when no models provided', () => {
    const { queryByTestId } = render(<ARComparisonOverlay modelA={null} modelB={null} />);
