...
beforeEach(() => {
  ...
  mockUseRecentlyViewed.mockReturnValue({ recentProducts: [], addViewed: jest.fn(), clearAll: jest.fn(), count: 0 });
  mockUseStreak.mockReturnValue({ streak: 0, loading: false, wasExtendedToday: false, longestStreak: 0 });
  mockUseActiveChallenges.mockReturnValue({ challenges: [], loading: false, error: null, refresh: mockRefreshChallenges });
});
...
describe('streak badge visibility', () => {
  it('shows streak badge when streak > 0 and not loading', () => {
    mockUseStreak.mockReturnValue({ streak: 5, loading: false, wasExtendedToday: false, longestStreak: 5 });
    const { getByTestId } = renderHomeScreen();
    expect(getByTestId('home-streak-badge')).toBeTruthy();
  });

  it('hides streak badge while streak is loading', () => {
    mockUseStreak.mockReturnValue({ streak: 7, loading: true, wasExtendedToday: false, longestStreak: 7 });
  ...
  it('streak badge shows correct streak count', () => {
    mockUseStreak.mockReturnValue({ streak: 12, loading: false, wasExtendedToday: true, longestStreak: 12 });
    const { getByTestId } = renderHomeScreen();
