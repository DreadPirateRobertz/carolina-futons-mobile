...
describe('pipeline config USDZ settings', () => {
  ...
  beforeAll(() => {
    config = JSON.parse(fs.readFileSync(path.join(PIPELINE_DIR, 'pipeline.config.json'), 'utf-8'));
  });
...
describe('models3d USDZ URLs', () => {
  ...
  beforeAll(() => {
    models3dContent = fs.readFileSync(path.join(SRC_DATA_DIR, 'models3d.ts'), 'utf-8');
  });
